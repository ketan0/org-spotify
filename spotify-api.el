...
(defcustom org-spotify-api-locale "en_US"
...
(defcustom org-spotify-api-country "US"
...
(defcustom org-spotify-oauth2-callback-port "8080"
  "The port for the httpd to listen on for the OAuth2 callback."
  :group 'org-spotify
  :type 'string)

(defcustom org-spotify-oauth2-callback-endpoint "/spotify-callback"
...
(defconst spotify-oauth2-callback  (concat "http://localhost:" org-spotify-oauth2-callback-port org-spotify-oauth2-callback-endpoint))
...
(defun spotify-start-httpd ()
  ...
  (let ((is-already-running (spotify-httpd-process-status)))
    (unless is-already-running
      (setq httpd-port org-spotify-oauth2-callback-port)
      (httpd-start))
    is-already-running))
...
Call CALLBACK with PAGE of items that match QUERY, depending on TYPE."
  (let ((offset (* org-spotify-api-search-limit (1- page))))
    (spotify-api-call-async
     "GET"
     (concat "/search?"
             (url-build-query-string `((q      ,query)
                                       (type   ,type)
                                       (limit  ,org-spotify-api-search-limit)
                                       (offset ,offset)
                                       (market from_token))
                                     nil t))
     nil
  ...
  (let ((offset (* org-spotify-api-search-limit (1- page))))
    (spotify-api-call-async
     ...
     (concat "/browse/featured-playlists?"
             (url-build-query-string `((locale  ,org-spotify-api-locale)
                                       (country ,org-spotify-api-country)
                                       (limit   ,org-spotify-api-search-limit)
                                       (offset  ,offset))
                                     nil t))
     nil
  ...
  (let ((offset (* org-spotify-api-search-limit (1- page))))
    (spotify-api-call-async
     ...
     (concat (format "/users/%s/playlists?" (url-hexify-string user-id))
             (url-build-query-string `((limit  ,org-spotify-api-search-limit)
                                       (offset ,offset))
                                     nil t))
     nil
     callback)))
...
(defun spotify-api-playlist-tracks (playlist page callback)
  ...
  (let ((owner (spotify-get-playlist-owner-id playlist))
        (id (spotify-get-item-id playlist))
        (offset (* org-spotify-api-search-limit (1- page))))
    (spotify-api-call-async
     "GET"
